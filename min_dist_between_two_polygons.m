function [min_d,min_dis_total] = min_dist_between_two_polygons(P1,P2,point_center,ifconter,Display_solution)
%
...
% D1每一行代表一个P1的点，每一列代表到P2各个线段的距离
[min_d1,ind_seg1] = min(D1);
[min_D1,ind_pt1 ] = min(min_d1);

% 记录当前是哪个点距离最近 ind_pt1 为距离P2那个线段最近
% ind_seg1[ind_pt1] 记录的是P1的哪个点距离P2最近
nearest_seg1 = ind_pt1;             % P2中距离P1某点最近的线段索引
nearest_point1 = ind_seg1(ind_pt1); % P1中点距离P2线段最近的点索引
...
% 记录当前是哪个点距离最近 ind_pt2 为距离P1那个线段最近
% ind_seg2[ind_pt2] 记录的是P2的哪个点距离P1最近
nearest_point2 = ind_pt2;         % P2点中距离P1线段最近的点索引
nearest_seg2 = ind_seg2(ind_pt2); % P1中线段距离P2点最近的线段索引

[min_d,ind_tot] = min([min_D1,min_D2]);
min_d = sqrt(min_d);

if (ind_tot == 1)
    % 第一个面的点距离第二个面的线段有最近距离，计算第一个面的外接矩形中心到第二个面点的最近距离
    Q1 = [P2.x(nearest_seg1),P2.y(nearest_seg1)]';
    Q2 = [P2.x(nearest_seg1+1),P2.y(nearest_seg1+1)]';
    P = point_center';
    min_dis_total = abs(det([Q2-Q1,P-Q1]))/norm(Q2-Q1);
    
    % P-点坐标；Q1, Q2线上两点坐标,P,Q1,Q2都是列向量 
    % d = abs(det([Q2-Q1,P-Q1]))/norm(Q2-Q1);
else
    % 第二个面的点距离第一个面的线段有最近距离，计算第二个面的外接矩形中心到第一个面线段的最近距离
    % 找到第一个面的最短距离的线段，及其对应的两端点，计算第二个面到中心点的最短距离
    
    % 获取第一个面的相应点
    point_first_near = [P2.x(nearest_seg2),P2.y(nearest_seg2)];
    % 求点到第二个面中心点的距离
    min_dis_total = norm(point_center - point_first_near);
end
