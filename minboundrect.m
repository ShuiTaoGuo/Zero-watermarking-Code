function [rectx,recty,area,perimeter,index,rec_len_wid,roate] = minboundrect(x,y,metric)
...
%   plot(xyr(:,1),xyr(:,2),'r');
  ...
  [xymin ,indexmin] = min(xyr,[],1);
  [xymax ,indexmax] = max(xyr,[],1);
  %计算当前最小外接矩形的宽、长
  len_wid = [xymax - xymin];
  
  indexmin = edges(indexmin);
  indexmax = edges(indexmax);
  ...
  if M_i < met
    ...
    roate = edgeangles(i);
    % 记录凸包上点的索引
    index_edge_points = [edges(i),edges(i+1)];
    %记录构成当前最小外接矩形的点的索引和长宽
    index = [indexmin',indexmax',index_edge_points];
    rec_len_wid = len_wid;
    % xy的最小点，[x的最大，y的最小]，xy的最大点，[x的最小，y的最大]，xy的最小点
    % 左下、右下、右上、左上、左下
    rect = [xymin;[xymax(1),xymin(2)];xymax;[xymin(1),xymax(2)];xymin];
    % 将获取的外接矩形坐标旋转，记录真实外接矩形坐标值
    rect = rect*rot';
    rectx = rect(:,1);
    recty = rect(:,2);
  end
...
% 获得长、宽及长宽比
if rec_len_wid(1) > rec_len_wid(2)
    rec_len_wid = [rec_len_wid(1), rec_len_wid(2), rec_len_wid(1)/rec_len_wid(2)];
else
    rec_len_wid = [rec_len_wid(2), rec_len_wid(1) , rec_len_wid(2)/rec_len_wid(1)];
end
% 索引去重,排序
index = unique(index);
index = sort(index);
